...
rs.on('end', function () {
   // ws.write(keyword+"总数："+count + CRLF);
    ws.write('[un_keywords_count :'+ctkey+"]"+ CRLF);
    var term = [];
    for(v in keys){
      term.push(v);
    };
    ...
    ws.end(function () {
      ...
      console.log('数据跑步完成！共收集任意两个关键字组合'+ctkey+'个');

      for (var i = term.length - 1; i >= 0; i--) {

      console.log("任意两个关键字组合： '"+term[i]+" '的统计量为： "+keys[term[i]]+CRLF);
      };
