/*分关键词categorys统计
  分组交叉
*/
var output = './yema/x_cookieCount_jr_ym.data';
var key_path1 ="./yema/cookie_keywords_1.data";
//1CH03SJp00BppvQaybW1  更换机油,汽油添加剂,行车记录仪
var key_path2 = "./yema/cookie_keywords_2.data";
...
var keys = {};
var ckeys = {};
var rs = fs.createReadStream(key_path1, {
    flags: 'r',
    encoding: 'utf-8'
});

var cs = fs.createReadStream(key_path2, {
    flags: 'r',
    encoding: 'utf-8'
});
...
rs.on('end', function () {
    ...
    cs.on('end', function () {
       ...
       for(v in ckeys){
          term.push(v);
        ws.write(v+"\t"+ckeys[v]+CRLF);

       }
      console.log("重合数据个数："+term.length+CRLF);
      //ws.write("重合数据个数："+term.length+CRLF);
      ws.end(function () {
